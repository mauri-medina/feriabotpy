...
class CelebrationDate(db.Base):
    ...
    def get_next_from_date(from_date):
        return (
            db.session.query(CelebrationDate)
...
@event.listens_for(
    CelebrationDate.__table__, "after_create"
)  # it only runs once when the database doesn't yet exist
def init_db(*args, **kwargs):
    new_year = Holiday(
        name="Año nuevo", message="¡Feliz año nuevo!", date=get_date(1, 1, 2022)
    )
    ...
    christmas = Holiday(
        name="Navidad", message="¡Feliz Navidad!", date=get_date(25, 12, 2022)
    )
    db.session.add(christmas)

    census_day = Holiday(
        name="Censo Nacional",
        message="Hoy se realiza el censo nacional, a quedarse todos en casa",
        date=get_date(9, 11, 2022),
    )
    ...
    db.session.add(CelebrationDate(holiday=heroes_day, date=get_date(3, 3, 2025)))
    ...
    # 2026
    # ------------------------------------------------------------------------------
    db.session.add(CelebrationDate(holiday=new_year, date=get_date(1, 1, 2026)))
